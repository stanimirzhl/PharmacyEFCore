...
namespace Pharmacy.Core
{
	public class PharmacyController
	{
		...
		public async Task<string> GetPrescriptionsByMedicineName(int medicineId)
		{
			...
			if (prescriptions.Count == 0)
			{
				throw new NonExistentEntity("No prescriptions have the desired medicine, try again later!");
			}
			...
			{
				sb.AppendLine($"Prescription ID: {prescription.Id}, Date: {prescription.PrescribedAt.ToString("dd/MM/yyyy")}, Doctor: {prescription.Doctor.DoctorName}, Patient: {prescription.Patient.PatientName}");
			}
		...
		public async Task<string> GetUnorderedMedicines()
		{
			...
			if (medicines.Count == 0)
			{
				throw new ArgumentException("Every medicine has been ordered or there aren't any medicines availabe yet!");
			}
		...
		public async Task<string> GetLowOnStockMedicinesInPharmacy()
		{
			...
			if (pms.Count == 0)
			{
				throw new ArgumentException("No medicines with stock quantity under 10!");
			}
		...
		public async Task<string> GetOldPatients()
		{
			...
			if (patients.Count == 0)
			{
				throw new ArgumentException("No patients older than 60!");
			}
			...
			{
				sb.AppendLine($"-{patient.PatientName}, Date of Birth: {patient.DateOfBirth.ToString("dd/MM/yyyy")}");
			}
		...
		public async Task<string> GetOrdersInTheLast30Days()
		{
			...
			if (orders.Count == 0)
			{
				throw new ArgumentException("No orders in the last 30 days!");
			}
		...
		public async Task<string> GetAllManufacturersWithEmailEndingInBg()
		{
			...
			if (manufacturers.Count == 0)
			{
				throw new ArgumentException("No manufacturers with email ending in .bg!");
			}
